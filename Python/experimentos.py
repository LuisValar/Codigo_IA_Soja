...
class_names = ['Antracnose', 'Antracnose2']
##
	...
	for file in glob.glob("*.jpg"):
		...
		if re.match('Antracnose.*', file):
			...
			if invert_image:
				...
				labels.append(0)
				labels.append(0)
		elif re.match('Antracnose2.*', file):
			images.append(img.reshape(reshape_size))
			labels.append(1)
			if invert_image:
				images.append(inv_image.reshape(reshape_size))
...
(train_images, train_labels) = load_image_dataset(
	path_dir='C:\Users\luisv\OneDrive\Área de Trabalho\TCC\Codigo_TCC\DataSet\Soja_Soybean_Antracnose_Anthracnose',
	maxsize=maxsize,
	reshape_size=(maxsize_w, maxsize_h, 1),
	invert_image=False)
#local da imagem de teste
(test_images, test_labels) = load_image_dataset(
	path_dir='C:\Users\luisv\OneDrive\Área de Trabalho\TCC\Codigo_TCC\teste-compara',
	maxsize=maxsize,
	reshape_size=(maxsize_w, maxsize_h, 1),
	invert_image=False)
